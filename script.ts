...
const doselhaId = document.getElementById("doselha") as HTMLInputElement;
const volumelhaId = document.getElementById("volumelha") as HTMLInputElement;
const volumeId = document.getElementById("volume") as HTMLInputElement;
const resultId = document.getElementById("result") as HTMLButtonElement;

function handleThousandLiters(volume: string): string {
...
function handlerOnSubmit(event: Event) {
...
function dosageInLiter(doselha: string, volumelha: string, volumeOfWater: string): number {
  const D = (Number(doselha) / Number(volumelha)) * Number(volumeOfWater); // l
  console.log({ D: D + "L" });
  return D;
}

function dosageInMilliliter(doselha: string, volumelha: string, volumeOfWater: string): number {
...
function milliliterPerLiter(dosageInMilliliter: number, PumpWaterVolume: string): number {
